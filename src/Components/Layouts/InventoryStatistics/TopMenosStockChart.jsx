...
export const TopMenosStockChart = () => {
    const [data, setData] = useState([]);
    const [activeIndex, setActiveIndex] = useState(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const fetchMenosStock = async () => {
            try {
    ...
    if (loading) {
        return <p>Cargando gráfico de menos stock...</p>;
    }

    return (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
            <h2 className="text-xl font-semibold mb-4">Top 5 Productos con Menos Stock</h2>
